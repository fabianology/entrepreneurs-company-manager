...
## 46. Preventing Server-Side VAD Timeouts in Full-Duplex Audio Streams
- **The Issue**: When implementing a half-duplex constraint on a full-duplex WebSocket audio stream (e.g. Gemini Live), completely halting the transmission of client audio packets while the AI is speaking will trigger an undocumented Voice Activity Detection (VAD) timeout on the server, causing the WebSocket to suddenly disconnect.
- **The Native Solution**: Instead of dropping or halting the audio packets when the microphone is logically 'muted' during AI playback, continue sending a steady stream of pure silence. By initializing a zero-filled `Data(count: dataSize)` buffer and transmitting it at the same 100ms interval, the WebSocket remains actively utilized and the server's VAD registers the continuous silence without triggering a timeout or an echo feedback loop.

## 47. Preventing Ghost Records in CRUD Form Sheets
- **The Issue**: Architecting UI "Add" buttons to invoke ViewModel methods that immediately instantiate, `.append()`, and `insert()` records into the database *before* the user interacts with the form creates orphaned "ghost" records. If the user cancels or dismisses the sheet without saving, a blank row remains permanently persisted in SwiftData/Supabase.
- **The Native Solution**: Decouple instantiation from insertion. Ensure that creation functions (e.g., `addSubscription()`) exclusively return a locally initialized model in memory. Delay the `.append()` and asynchronous `insert()` operations entirely until the user explicitly commits the form via the `save()` function.
