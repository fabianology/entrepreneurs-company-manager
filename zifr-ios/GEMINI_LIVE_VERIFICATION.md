...
## Orb feedback and Gemini tap target

- Speaking uses soft amber energy that follows the orb's contour, starts at its surface, and fades outward. Listening uses a close blue aura that gathers into the shell. Playback state and microphone readiness select the animation; mute/inactive states rest, and Reduce Motion uses static feedback. No visible state labels were added.
- Gemini's action-bar button has an explicit 52 × 52-point rectangular hit area, including the transparent corners around the 44-point visual. Decorative glass no longer participates in hit testing. The top Executive Brief code is unchanged.
- Simulator and device builds passed. Both existing visual checks passed; exported listening/speaking previews were inspected. Ordinary simulator clicks at opposite corners of the Gemini target each opened the voice sheet; sessions were ended after verification.

## Smooth transitions and pitch-reactive rings

- Listening/speaking weights now blend over 700 ms, retargeting from the visible blend if interrupted. A shared surface clock prevents the mesh from jumping when the mode changes. Blue rings flow inward and dissolve into the orb while amber energy fades outward.
- A local, bounded 64 ms pitch tracker uses normalized autocorrelation over downsampled microphone audio. Voiced pitch (approximately 70–500 Hz) adjusts listening-ring brightness/thickness, with smoothing and a silence/noise gate. This is approximate visual feedback; no new audio transmission or storage was added. Analysis history resets when the capture token changes. Method background: https://www.isca-archive.org/interspeech_2018/lin18_interspeech.html
- Signed simulator/device builds passed. The focused suite ran 28 tests: 25 passed, three opt-in live-service/device checks skipped, zero failures. New checks cover transition reversal without a jump, known pitch at different amplitudes, and silence/noise/mute resets. Listening and midway-transition render attachments were visually reviewed. Actual pitch response to the user's voice on FX remains a device acceptance check.
