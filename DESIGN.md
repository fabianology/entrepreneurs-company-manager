...
- The Plaid review imports the connected bank's favicon-style logo into the institution header. Its account rows expose account name, type, masked identifier, balance, green selection state, and an accessible selected/not-selected label without a second decorative account icon. Selection is controlled directly from each row; the header does not add a separate Clear/Select all action.
